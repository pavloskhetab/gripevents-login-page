...
function getUsers(records, action, start_time, end_time) {
  ...
  for (var i = 0, length; i < length; i++) {
    ...
    if (
      ...
      object.date_actioned >= start_time &&
      object.date_actioned <= end_time
    ) {
...
function getPlaybackTime(user, records) {
  ...
  for (var i = 0, length; i < length; i++) {
    ...
    if (object.user_id == user && object.action == "start") {
      ...
      if (countStart == 1) {
        // Added a condition to stop storing further date_actioned values inside the startTime variable
